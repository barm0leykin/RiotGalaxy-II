...
грунт/камикадзе-таран/тяжёлый-Косморика/полевой командир). Прежних подклассов
...
`BonusType` (`HP_UP` — хил, `BULLET_UP` — врем. усиление урона, `NUKE_BOMB` — убить всех) — один
класс `Bonus`, спрайт по типу, эффект в базовом `Apply`. `BonusStar` — отдельный класс: даёт
**кредиты** (несёт номинал = `reward` убитого врага), притягивается магнитом. Падают вниз; при
подборе — `Apply(player)`. При гибели врага (`SpawnBonusOnEnemyDeath`) награда дробится на
**несколько звёзд** (число = `reward / starValue`, тяжёлые враги сыплют больше; разлёт от точки),
плюс шанс баффа/усиления. После зачистки уровня даётся **окно сбора** (`levelClearCollectSeconds`)
перед переходом — `GameManager.UpdateGameplay` (`_levelClearTimer`).
