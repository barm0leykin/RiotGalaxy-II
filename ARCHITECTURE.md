...
│   ├── ship.png, wpn_* (снаряды/оружие), btn_* (иконки), shield.png …
...
GREEN, RED, BOSS, UKRO, KAMIK, HEAVY, UKRO_BOSS, KORMA, BRIZ, TRAPP, REAPER, OVERMIND }`;
UKRO..UKRO_BOSS — укропитеки Акта II, KORMA/BRIZ + боссы TRAPP/REAPER/OVERMIND — Акт III.
Боссов определяет `Enemy.IsBossType` (фазовый ИИ/HP-шкала/реплики/взрыв). Прежних подклассов
...
- **Стрельба**: по таймеру `ShootInterval`, режим из конфига `shoot`: `down` (вниз),
  `aim` (прицельно в игрока), `none` (не стреляет). Прицельный выстрел зажат **конусом вниз**
  (±`GameOptions.EnemyAimMaxDeg`, options.yaml `combat.aimMaxDeg`, деф. 55°) — иначе враг на уровне
  игрока бил бы почти горизонтально (не увернуться); то же для прицельных залпов босса. Флаг
...
(снова `FormationMovement`; бумеранг уходит через верх). **Тактика пике** выбирается случайно из
списка `tactics` типа врага (`enemies.yaml`): `random` (вниз с отскоком), `snake` (змейка),
`ram` (таран), `ellipse` (петля), `zigzag` (треугольная волна), `spiral` (штопор), `swoop`
(дайв-бомба), `strafe` (спуск→поперёк→вниз), `homing` (самонаведение на игрока), `boomerang`
(нырок с возвратом вверх). Скорость вылета — `attackSpeed`; страховка `MaxDiveTime` от зависания.
...
В строю **и на маршрутах** (`RouteMovement`) все двигаются с **единой** скоростью
`EnemyConfig.FormationSpeed()` (не случайной `PickSpeed`), иначе юниты на одной линии наезжают
друг на друга. Случайная `CurrentSpeed`/`attackSpeed` остаётся для свободного движения и вылетов.
